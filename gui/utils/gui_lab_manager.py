    ...
    def _open_docker_terminal(self):
        ...
        try:
            terminal_commands = [
                ...
                'echo "  sudo docker run --rm -it --network host --cap-add=NET_ADMIN --cap-add=NET_RAW -e SPOOFED_SUBNET=10.10.123.0/25 -e RETURN_ADDR=10.135.97.2 --name sip-victim asterisk-sip-server"',
                'echo',
            ...
            def start_thread():
                try:
                    ...
                    if "sudo" in error_msg.lower() or "permission" in error_msg.lower():
                        ...
                        print_error("Terminal connection failed. Container may be starting in background.")
                    else:
                        self._update_status(f"Lab start failed: {error_msg}")
                finally:
                    self._is_starting = False
