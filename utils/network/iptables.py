...
def ensure_chain_and_anchor(anchor_chain: str = "INPUT", table: str = "filter", suid: str = "anchor", preserve: bool = False) -> None:
    """
...
def ensure_nat_chain_and_anchor(anchor_chain: str = "OUTPUT", suid: str = "anchor", preserve: bool = False) -> None:
    """
...
def add_nfqueue_rule_tagged(queue_num: int, dst_port: int, suid: str, anchor_chain: str = "INPUT", preserve: bool = False) -> bool:
    """
    ...
    """
    ensure_chain_and_anchor(anchor_chain=anchor_chain, table="filter", suid=suid, preserve=preserve)
...
def remove_rules_for_suid(suid: str, table: str = "filter", chain: str = STORMSHADOW_CHAIN) -> int:
    ...
    for line in lines:
        if "-m comment --comment" in line and COMMENT_PREFIX in line and suid in line:
            ...
            try:
                run_command_str(
                    f"iptables -t {table} {delete}", capture_output=False, check=True, want_sudo=True
                )
                removed += 1
...
def remove_all_rules_for_suid(suid: str) -> int:
    """
    ...
    for anchor_chain in ["INPUT", "OUTPUT", "FORWARD"]:
        ...
        for line in lines:
            if f"-j {STORMSHADOW_CHAIN}" in line and "-m comment --comment" in line and COMMENT_PREFIX in line and suid in line:
                ...
                try:
                    run_command_str(f"iptables -t filter {delete_rule}", capture_output=False, check=True, want_sudo=True)
                    removed_total += 1
    ...
    for anchor_chain in ["OUTPUT", "PREROUTING", "POSTROUTING"]:
        ...
        for line in lines:
            if f"-j {STORMSHADOW_NAT_CHAIN}" in line and "-m comment --comment" in line and COMMENT_PREFIX in line and suid in line:
                delete_rule = line.replace("-A", "-D", 1)
                try:
                    run_command_str(f"iptables -t nat {delete_rule}", capture_output=False, check=True, want_sudo=True)
                    removed_total += 1
    ...
    removed_total += remove_rules_for_suid(suid, table="filter", chain=STORMSHADOW_CHAIN)
    removed_total += remove_rules_for_suid(suid, table="nat", chain=STORMSHADOW_NAT_CHAIN)
    
    return removed_total


def cleanup_stale_heartbeats(ttl_seconds: int = DEFAULT_TTL_SECONDS, heartbeat_dir: str = DEFAULT_HEARTBEAT_DIR) -> int:
    """
...
def cleanup_stale_rules(ttl_seconds: int = DEFAULT_TTL_SECONDS, heartbeat_dir: str = DEFAULT_HEARTBEAT_DIR) -> int:
    """
    ...
    removed_hb = cleanup_stale_heartbeats(ttl_seconds, heartbeat_dir)
    if removed_hb > 0:
    ...
    for line in _iptables_S(chain=STORMSHADOW_CHAIN, table="filter"):
        if "-m comment --comment" in line and COMMENT_PREFIX in line:
            ...
            candidate = should_remove(comment_text)
            if candidate:
                removed_total += remove_rules_for_suid(candidate, table="filter", chain=STORMSHADOW_CHAIN)
    ...
    for line in _iptables_S(chain=STORMSHADOW_NAT_CHAIN, table="nat"):
        if "-m comment --comment" in line and COMMENT_PREFIX in line:
            ...
            candidate = should_remove(comment_text)
            if candidate:
                removed_total += remove_rules_for_suid(candidate, table="nat", chain=STORMSHADOW_NAT_CHAIN)

    return removed_total


def has_ipset() -> bool:
    """Return True if ipset command is available."""
    try:
...
def ensure_ipset_set(name: str, set_type: str = "bitmap:port", timeout: int = DEFAULT_TTL_SECONDS) -> bool:
    ...
    try:
        run_command_str(
            f"ipset create {name} {set_type} timeout {timeout}", capture_output=False, check=True, want_sudo=True
        )
...
def ipset_add_port(name: str, port: int, timeout: int = DEFAULT_TTL_SECONDS) -> bool:
    """Add or refresh a port in the ipset with a timeout (auto-expires)."""
    try:
        run_command_str(
            f"ipset add {name} {port} timeout {timeout} -exist", capture_output=False, check=True, want_sudo=True
        )
        return True
    except subprocess.CalledProcessError as e:
...
def ensure_nfqueue_rule_using_ipset(queue_num: int, suid: str, anchor_chain: str = "INPUT", set_timeout: int = DEFAULT_TTL_SECONDS) -> Optional[str]:
    """
    ...
    if not has_ipset():
        ...
        return None
    ensure_chain_and_anchor(anchor_chain=anchor_chain, table="filter", suid=suid, preserve=False)
    set_name = f"stormshadow_ports_{suid}"
    if not ensure_ipset_set(set_name, set_type="bitmap:port", timeout=set_timeout):
        return None
    # Ensure the single NFQUEUE rule exists
    check_cmd = (
    ...
    try:
        run_command_str(add_cmd, capture_output=False, check=True, want_sudo=True)
...
def ipset_destroy(name: str) -> None:
    try:
        run_command_str(f"ipset destroy {name}", capture_output=False, check=True, want_sudo=True)
    except subprocess.CalledProcessError:
        pass
...
def activate_return_path(
    ...
    try:
        ensure_nat_chain_and_anchor(anchor_chain="OUTPUT", suid=suid or "anchor", preserve=False)
...
def deactivate_return_path(
    ...
    try:
        ...
        if suid:
            ...
            run_command_str(cmd, capture_output=False, check=True, want_sudo=True)
            return True
        # Fallback: try without comment in our chain
        print_debug(f"Deactivating return path (untagged) with command: {base}")
        run_command_str(base, capture_output=False, check=True, want_sudo=True)
        return True
    except subprocess.CalledProcessError as e:
